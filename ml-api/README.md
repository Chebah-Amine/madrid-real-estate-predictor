...
Additionally, the PSID project has a secondary objective of developing and training predictive models from this data. A neural network is implemented to provide predictive analyses and additional insights.
...
```
    - cd && mkdir psid-project && cd psid-project && git clone https://gitlab.com/psid-project/psid-ml.git
```
...
INSTALL 💻 (create/run container and install dependencies)

```
    make install
```

START 🚩 (start the server port 7000)
...
GENERATE DEFAULT VALUES ‍📚️ (generate default values (to complete the input for the prediction) in app/static/dataset)

```
    make pre-processing
```

TRAIN MODEL 🏋️‍♂️ (train the model and save it in app/static/model)
